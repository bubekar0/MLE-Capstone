...
- `references/` : Literature referenced in the write-up.
- `images/` : Figures included in the write-up.
- `predictions/` : Submissions made to the Kaggle competition in CSV format.
- `Capstone.ipynb` : Full capstone project in a Jupyter Notebook.
- `Capstone Project Report.pdf` : Write-up covering implementation steps and conclusions.
- `data_exploration.py` : Python routines for data exploration and preprocessing.
- `unsupervised_ML.py` : Python code for PCA analysis, K-Means segmentation, and cluster dissection.
- `requirements.txt` : Python dependencies.

## Results

After exploring and cleaning the data, but before any ML techniques were applied, the datasets were compared to spot any marked differences in feature distributions. From this analysis, a broad customer profile emerged, describing customers as wealthy and sedentary nature lovers, mostly older West German men living without children at home.

With unsupervised ML techniques, the complexity of the problem was made more tractable. The number of variables was reduced from 366 to 195 by means of PCA. Using K-Means clustering, the near 1M observations were collapsed into four clusters that crystallized what made customers stand out from the general population. Two segments in particular were heavily representative of customers: wealthy older West German men, and money-savvy wealthy men. A caveat was noted that these conclusions may have been influenced by the manner in which the "year of birth" feature was imputed, and that features lacking description had to be excluded from customer profiling.

The supervised learning portion presented its own challenge: the training data was heavily imbalanced. ROC AUC was selected as the metric of choice, several classifiers were evaluated, and a Gradient Boosting Machine was selected for tuning. After parameter tuning, predictions were submitted to the Kaggle competition, achieving a score of **78.53%**.

## Acknowledgements

- [Udacity](https://www.udacity.com) — Machine Learning Engineering Nanodegree, Capstone Project.
